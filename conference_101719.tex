...
\begin{abstract}
在静态网络中，链接预测用于发现缺失的链接，而在动态网络中，链接预测用于预测未来可能出现的链接。与静态图不同的是, 动态图中不同时刻的图的节点的链接装态会发生变化。
近年来，盛行的在线服务产生了大量的用户活动数据。服务提供商收集这些数据是为了进行客户行为分析，并提供更好和更定制的服务。这些数据的大部分可以被建模并存储为图，如Facebook的社交图，Youtube的用户视频互动图。这些图需要随着时间的推移而演变，以捕捉现实世界的动态，这导致了动态图的发明。在本文中，我们将几种现有的图神经网络模型在不同的数据集上进行链路预测。我们对比了多种不同的动态图链接预测方法, 比如: NetWalk、WD-GCN/CD-GCN 和 EvolveGCN，并对动态图的相关数据集进行了总结。
\end{abstract}
...
\subsection{研究方法}
在近年来的研究中, 很多链路预测方法取得了发展, 在静态图上研究者们提出了很多高精度的链接预测方法。比如基于启发式算法的共同邻居算法\cite{zhang2008recommendation}, 以及首次将GNN应用在图的链接预测的SEAL模型[], SEAL模型
...
测试 \cite{zhang2008recommendation}
